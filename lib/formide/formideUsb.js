...
function usb(){
    return{
        ...
        getControlMode:function(callback){return getControlMode(callback)}

    }
}

function getControlMode (callback){

    HttpHelper.doHttpRequest("GET", "/api/printer/control_mode",{},function(err,response){
        if(err)
        {
            console.log("Error (formide.js) get control mode: ",err)
        ...
        if(response)
        {
            console.log("Response (formide.js) get control mode: ",response)
