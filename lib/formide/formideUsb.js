...
function updateDriveFilesFromPath(driveUnit, drivePath, callback){
    ...
    console.log("FormideUSB: Update drive files from path")
...
function copyFile(drive, path, callback) {
    ...
    HttpHelper.doHttpRequest("POST", "/plugins/com.printr.usb-drive/api/drives/"+driveNameEncoded+"/copy", JSON.stringify(data), function (err, list) {
